...
import { BasicOverlay, OverlayBackdrop, OverlayColors, OverlayWithContent, OverlayToggle } from "./overlay-demos";

const VARIANTS = ["basic", "backdrop", "colors", "content", "toggle"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicOverlay />}
        {variant === "backdrop" && <OverlayBackdrop />}
        {variant === "colors" && <OverlayColors />}
        {variant === "content" && <OverlayWithContent />}
        {variant === "toggle" && <OverlayToggle />}
      </div>
    </div>
  );
}
