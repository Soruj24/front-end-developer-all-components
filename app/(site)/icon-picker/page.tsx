...
export default function Example() {
  return <IconPicker onSelect={(icon) => console.log(icon)} />;
}`;
...
export default function IconPickerPage() {
  ...
  const filtered = iconSets.filter((i) => i.label.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-10 p-6 sm:p-10 lg:p-14">
      <header className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">Icon Picker</h1>
          <Badge variant="primary">Input</Badge>
        </div>
        <p className="max-w-2xl text-pretty text-[15px] leading-relaxed text-muted-foreground">
          Searchable icon picker grid with search filtering, preview on hover, and selection for icon choice.
        </p>
      </header>
      ...
      <section className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3">
          <h3 className="text-lg font-medium text-foreground">Icon Grid</h3>
          <ComponentPreview id="icon-picker-default">
            <div className="grid w-full max-w-md grid-cols-5 gap-2">
              {iconSets.map((icon) => (
                <button key={icon.name} className="flex h-10 w-10 items-center justify-center rounded-md border border-border hover:bg-muted hover:border-primary transition-colors">
        ...
        <div className="flex flex-col gap-3">
          ...
          <ComponentPreview id="icon-picker-search">
            <div className="w-full max-w-md">
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search icons..." className="mb-3" />
              <div className="grid grid-cols-5 gap-2">
                {filtered.map((icon) => (
                  <button key={icon.name} className="flex h-10 w-10 items-center justify-center rounded-md border border-border hover:bg-muted transition-colors">
                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>{icon.svg}</svg>
                  </button>
                ))}
              </div>
        ...
        <div className="flex flex-col gap-3">
          <h3 className="text-lg font-medium text-foreground">Selectable</h3>
          <ComponentPreview id="icon-picker-interactive">
            <div className="w-full max-w-md">
              <div className="mb-2 grid grid-cols-5 gap-2">
                {iconSets.map((icon) => (
                  <button key={icon.name} onClick={() => setSelected(icon.name)} className={`flex h-12 w-12 items-center justify-center rounded-lg border transition-all ${selected === icon.name ? "border-primary bg-primary/10 text-primary" : "border-border hover:bg-muted"}`}>
                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>{icon.svg}</svg>
                  </button>
                ))}
              </div>
              {selected && <p className="text-sm text-muted-foreground">Selected: <span className="font-medium text-foreground">{selected}</span></p>}
            </div>
          </ComponentPreview>
        </div>
      </section>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="border-b border-border">
                ...
                <td className="px-4 py-3 text-muted-foreground">(icon: string) =&gt; void</td>
