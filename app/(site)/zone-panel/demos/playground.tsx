...
import { BasicPanel, PanelPositions, PanelWidths, PanelVariants, PanelWithContent } from "./panel-demos";

const VARIANTS = ["basic", "positions", "widths", "variants", "content"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicPanel />}
        {variant === "positions" && <PanelPositions />}
        {variant === "widths" && <PanelWidths />}
        {variant === "variants" && <PanelVariants />}
        {variant === "content" && <PanelWithContent />}
      </div>
    </div>
  );
}
