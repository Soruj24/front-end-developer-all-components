...
const features = [
  { title: "Fast Performance", description: "Built for speed with optimized loading times across all devices and networks.", icon: "⚡" },
  { title: "Responsive Design", description: "Looks great on every device and screen size from mobile to desktop.", icon: "📱" },
  { title: "Accessible", description: "Designed with accessibility best practices for all users.", icon: "♿" },
  { title: "Secure", description: "Enterprise-grade security with end-to-end encryption.", icon: "🔒" },
  { title: "Scalable", description: "Grows with your business from startup to enterprise.", icon: "📈" },
  { title: "Collaborative", description: "Real-time collaboration tools for your entire team.", icon: "🤝" },
]

const testimonials = [
  { name: "Sarah Chen", role: "CTO, TechStart", text: "This platform transformed our workflow. We shipped 3x faster within the first month." },
  { name: "Marcus Johnson", role: "Lead Developer, DevCo", text: "The best developer experience I've had in years. Everything just works." },
  { name: "Emily Rodriguez", role: "Product Manager, AppNova", text: "Our team productivity increased dramatically after switching to this platform." },
]

const logos = ["Acme Corp", "TechStart", "DevCo", "AppNova", "CloudBase", "DataFlow"]
...
export default function LandingPage() {
  ...
  const ctaSteps = [
    { title: "Ready to Get Started?", subtitle: "Join thousands of teams already using our platform.", action: "Get Started →" },
    { title: "Almost there!", subtitle: "Enter your email and we'll set you up.", action: "Continue", input: true },
    { title: "You're in! 🎉", subtitle: "Check your inbox for next steps.", action: "Done" },
  ]
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-10 p-6 sm:p-10 lg:p-14">
      ...
      <p className="text-muted-foreground">Full landing page with animated elements, stats, testimonials, and multi-step CTA.</p>
      ...
      <section className="relative flex flex-col items-center gap-6 overflow-hidden rounded-lg border bg-gradient-to-b from-zinc-50 px-6 py-20 text-center dark:border-border dark:from-zinc-900">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          <div className="absolute left-1/2 top-5 h-8 w-8 rounded-full bg-amber-400/30 blur-2xl animate-bounce" style={{ animationDuration: "3s" }} />
          <div className="absolute bottom-20 right-10 h-6 w-6 rounded-full bg-rose-400/30 blur-2xl animate-bounce" style={{ animationDuration: "4s", animationDelay: "0.5s" }} />
        </div>

        <h2 className="relative text-5xl font-bold tracking-tight">
          <span className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 bg-clip-text text-transparent animate-pulse" style={{ animationDuration: "3s" }}>
      ...
      <section className="flex flex-wrap items-center justify-center gap-8 rounded-lg border border-dashed px-6 py-8 dark:border-border">
        {logos.map((logo) => (
          <span key={logo} className="text-sm font-semibold tracking-wide text-muted-foreground/70 dark:text-muted-foreground">
            {logo}
          </span>
        ))}
      </section>
      ...
      <section className="flex flex-col items-center gap-6 rounded-lg border bg-zinc-900 px-6 py-16 text-center text-white dark:border-border dark:bg-muted">
        ...
        {ctaStep > 0 && (
          <button onClick={() => setCtaStep(0)} className="text-xs text-muted-foreground hover:text-zinc-300">
            Start over
          </button>
        )}
      </section>
