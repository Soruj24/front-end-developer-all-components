...
import { BasicGesture, GestureTypes, ZoomBounds, ZoomWithReset, ZoomPresets } from "./gesture-demos";

const VARIANTS = ["basic", "types", "bounds", "reset", "presets"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicGesture />}
        {variant === "types" && <GestureTypes />}
        {variant === "bounds" && <ZoomBounds />}
        {variant === "reset" && <ZoomWithReset />}
        {variant === "presets" && <ZoomPresets />}
      </div>
    </div>
  );
}
