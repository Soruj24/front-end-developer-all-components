...
import { BasicBadges, BadgeColors, BadgeSizes, BadgeWithDot, BadgeWithIcon, BadgeRemovable } from "./badge-demos";

const VARIANTS = ["basic", "colors", "sizes", "dot", "icon", "removable"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicBadges />}
        {variant === "colors" && <BadgeColors />}
        {variant === "sizes" && <BadgeSizes />}
        {variant === "dot" && <BadgeWithDot />}
        {variant === "icon" && <BadgeWithIcon />}
        {variant === "removable" && <BadgeRemovable />}
      </div>
    </div>
  );
}
