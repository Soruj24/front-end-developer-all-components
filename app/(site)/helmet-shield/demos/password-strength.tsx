    ...
    <div className="w-full max-w-sm">
      <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Password Strength</h3>
        </div>
        ...
        <div className="mt-3 space-y-1">
          {["At least 8 characters", "Uppercase & lowercase", "Numbers & symbols"].map((req) => (
            <div key={req} className="flex items-center gap-2 text-[10px] text-zinc-500 dark:text-zinc-400">
              <div
                className={`h-1.5 w-1.5 rounded-full ${
                  (password.length > 6 && req.includes("8")) ||
                  (/[A-Z]/.test(password) && req.includes("Uppercase")) ||
                  (/[0-9]/.test(password) && req.includes("Numbers"))
                    ? "bg-emerald-500"
