...
import { BasicWave, WaveColors, WaveSpeeds, WaveAmplitude, WaveFilled, WaveWithContent } from "./wave-demos";

const VARIANTS = ["basic", "colors", "speeds", "amplitude", "filled", "content"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicWave />}
        {variant === "colors" && <WaveColors />}
        {variant === "speeds" && <WaveSpeeds />}
        {variant === "amplitude" && <WaveAmplitude />}
        {variant === "filled" && <WaveFilled />}
        {variant === "content" && <WaveWithContent />}
      </div>
    </div>
  );
}
