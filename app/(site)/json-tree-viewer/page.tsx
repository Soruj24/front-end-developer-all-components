...
import { userProfile, apiResponse } from "@/components/json-tree-viewer/demo";
  ...
  return (
    <div className={cn("overflow-hidden rounded-xl border border-border bg-surface shadow-sm", className)}>
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 border-b border-border px-3 py-1.5 text-[10.5px] text-subtle">
        <span className="truncate">{totalNodes} values</span>
      </div>
...
const DEFAULT_EXAMPLE = `<JsonTreeViewer data={userProfile} title="User Profile" defaultExpandedDepth={2} />`;

const QUERY_EXAMPLE = `<JsonTreeViewer data={apiResponse} title="API Response" defaultExpandedDepth={2} />`;

const LIGHT_THEME_EXAMPLE = `<JsonTreeViewer data={userProfile} title="User Profile" defaultTheme="light" defaultExpandedDepth={2} />`;

const CUSTOM_HEIGHT_EXAMPLE = `<JsonTreeViewer data={apiResponse} title="API Response" height={320} defaultExpandedDepth={2} />`;
...
export default function JsonTreeViewerPage() {
  return (
    ...
    >
      ...
      <SourceCodeViewer source={JSONTREEVIEWER_SOURCE} filename="JsonTreeViewer.tsx" defaultExpanded />

      <div className="flex flex-col gap-6">
        <ExampleBlock title="Default" description="Basic collapsible tree with search." code={DEFAULT_EXAMPLE}>
          <JsonTreeViewer data={userProfile} title="User Profile" defaultExpandedDepth={2} height={460} />
        </ExampleBlock>

        <ExampleBlock title="With Query" description="Filter the tree by key or value with inline highlighting." code={QUERY_EXAMPLE}>
          <JsonTreeViewer data={apiResponse} title="API Response" defaultExpandedDepth={2} height={460} />
        </ExampleBlock>

        <ExampleBlock title="Light Theme" description="Opt into a light theme for the viewer surface." code={LIGHT_THEME_EXAMPLE}>
          <JsonTreeViewer data={userProfile} title="User Profile" defaultTheme="light" defaultExpandedDepth={2} height={460} />
        </ExampleBlock>

        <ExampleBlock title="Custom Height" description="Control the rendered height of the scrollable tree." code={CUSTOM_HEIGHT_EXAMPLE}>
