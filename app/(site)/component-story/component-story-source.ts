    ...
    <div className="w-full max-w-md">
      <div className="rounded-xl border border-black/[.08] bg-card shadow-sm overflow-hidden dark:border-white/[.145]">
        ...
        <div className="flex flex-wrap gap-1 border-b border-black/[.06] px-4 py-2 dark:border-white/[.1]">
          {variants.map((var_, i) => (
            <button
              ...
              className="rounded-lg px-3 py-1.5 text-xs font-medium transition-colors " + (activeVariant === i ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted")
            >
              {var_.name}
            </button>
          ))}
        ...
        <div className="flex items-center justify-center p-10">
          <button className="rounded-lg font-medium transition-colors " + (sizeClasses[v.props.size as keyof typeof sizeClasses] + " " + variantClasses[v.props.variant as keyof typeof variantClasses]) + "> Button</button>
        </div>
      ...
      <div className="rounded-xl border border-black/[.08] bg-card shadow-sm overflow-hidden dark:border-white/[.145]">
          ...
          <div className="bg-card p-3">
            <label className="flex items-center gap-2 text-xs">
              <button
                ...
                className="flex h-5 w-9 items-center rounded-full p-0.5 transition-colors " + (controls.disabled ? "bg-foreground" : "bg-muted")
              >
                <div className="h-4 w-4 rounded-full bg-background shadow-sm translate-x-4" /> Disabled
              </label>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-center p-8">
          <button
            ...
            className="flex items-center rounded-lg font-medium transition-all " + (sizeClasses[controls.size as keyof typeof sizeClasses] + " " + variantClasses[controls.variant as keyof typeof variantClasses] + " " + (controls.disabled || controls.loading ? "opacity-50 cursor-not-allowed" : "")) + "> {controls.loading ? "Loading..." : "Button"}</button>
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex items-start gap-0 mb-4">
          {steps.map((s, i) => (
            <div key={s.label} className="flex items-start">
              ...
              >
                <div className="flex h-8 w-8 items-center justify-center rounded-full transition-all " + (i <= activeStep ? "{s.color} text-white" : "bg-muted text-muted-foreground") + "> {i < activeStep ? <Check className="h-4 w-4" /> : <span className="text-xs font-bold">{i + 1}</span>}</div> <span className="text-[10px] font-medium " + (i === activeStep ? "text-foreground" : "text-muted-foreground") + ">" + s.label + "</span></div>
              </button>
              {i < steps.length - 1 && (
                <div className="mx-1 mt-4 h-0.5 w-8 " + (i < activeStep ? "bg-foreground/30" : "bg-muted") + ">"></div>
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="w-full max-w-md">
      <div className="rounded-xl border border-black/[.08] bg-card shadow-sm overflow-hidden dark:border-white/[.145]">
          ...
          <div className="flex gap-1">
            {themes.map((t) => (
              <button
                ...
                className="flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-medium transition-colors " + (theme === t.id ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted")
              >
        ...
        <div className="p-6 " + (isDark ? "bg-zinc-900" : "bg-zinc-50") + "">
          <div className="rounded-xl border p-4 shadow-sm " + (isDark ? "border-white/[.145] bg-zinc-800" : "border-black/[.08] bg-white") + "">
            <div className="flex items-center gap-3 mb-3">
              <div className="h-10 w-10 rounded-full " + (isDark ? "bg-zinc-700" : "bg-zinc-200") + " /> <div>
                <p className="text-sm font-semibold " + (isDark ? "text-white" : "text-zinc-900") + ">John Doe</p> <p className="text-xs " + (isDark ? "text-zinc-400" : "text-zinc-500") + ">john@example.com</p></div>
              </div>
              <div className="flex gap-2">
                <button className="rounded-lg px-3 py-1.5 text-xs font-medium " + (isDark ? "bg-white text-zinc-900" : "bg-zinc-900 text-white") + ">Follow</button> <button className="rounded-lg px-3 py-1.5 text-xs font-medium " + (isDark ? "border border-white/[.145] text-zinc-300" : "border border-black/[.08] text-zinc-600") + ">Message</button>
              </div>
            </div>
          </div>
        </div>
          ...
          {props.map((p) => (
            <div key={p.name}>
              ...
              >
                ...
                <ChevronDown className="h-3.5 w-3.5 text-muted-foreground transition-transform " + (expanded[p.name] ? "rotate-180" : "") + " />"
              </button>
              {expanded[p.name] && (
                <div className="border-t border-black/[.04] bg-muted/20 px-4 py-3 dark:border-white/[.06]">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-medium text-muted-foreground">Type:</span> <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] text-foreground">{p.type}</code>
                  </div>
                </div>
              )}
            </div>
            ...
            <div key={v.name} className="grid grid-cols-3 gap-2 mb-2 items-center">
              ...
              {sizes.map((s) => (
                <div key={s} className="flex justify-center">
                  <button className="rounded-lg font-medium transition-colors " + (s === "sm" ? "px-3 py-1 text-[10px]" : s === "lg" ? "px-5 py-2 text-sm" : "px-4 py-1.5 text-xs") + " " + v.class + "> Button</div>
                </div>
              ))}
            </div>
          ))}
      ...
      <div className="rounded-xl border border-black/[.08] bg-card shadow-sm overflow-hidden dark:border-white/[.145]">
        <div className="flex items-center justify-between border-b border-black/[.06] px-4 py-3 dark:border-white/[.1]">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[10px] font-medium transition-colors " + (showCode ? "bg-foreground text-background" : "bg-muted text-muted-foreground hover:bg-muted/80")
          >
        ...
        {showCode ? (
          ...
          <div className="flex items-center justify-center p-8">
            <button className="rounded-lg bg-foreground px-5 py-2 text-sm font-medium text-background">Button</button>
          </div>
        )}
      </div>
    </div>
