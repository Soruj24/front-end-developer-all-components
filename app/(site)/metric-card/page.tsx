...
const installCommand = `npx component-library@latest add metric-card`;

const usageCode = `import { MetricCard } from "@/components/metric-card";
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-10 p-6 sm:p-10 lg:p-14">
      <header className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">Metric Card</h1>
          <Badge variant="primary">Dashboard</Badge>
        </div>
        <p className="max-w-2xl text-pretty text-[15px] leading-relaxed text-muted-foreground">
          Display key metrics with trend indicators, sparkline charts, and icon accents. Perfect for dashboard KPI sections.
        </p>
      </header>
      ...
      <ComponentPreview id="metric-card-grid">
        <div className="grid w-full gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {metrics.map((m, i) => {
            ...
            return (
              <div
                key={m.title}
                className={`group relative flex flex-col gap-3 rounded-xl border border-border bg-card p-5 transition-all hover:shadow-md ${
                  hoveredCard === i ? "ring-2 ring-border" : ""
                }`}
      ...
      <ComponentPreview id="metric-card-detailed">
          ...
          <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-6">
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-border pt-4">
              <div>
                ...
                <p className="text-sm font-semibold text-foreground">$31,200</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Last Week</p>
                <p className="text-sm font-semibold text-foreground">$28,100</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Goal</p>
                <p className="text-sm font-semibold text-foreground">$35,000</p>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-4 rounded-xl border border-border bg-card p-6">
            ...
            <div className="flex items-center gap-2">
              <TrendingUp className="h-4 w-4 text-emerald-500" />
              <span className="text-sm font-medium text-emerald-500">+5.7%</span>
              <span className="text-sm text-muted-foreground">vs last month</span>
            </div>
            <div className="h-16">
      ...
      <ComponentPreview id="metric-card-minimal">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              <tr className="border-b">
                ...
                <td className="px-4 py-3 text-muted-foreground">string | number</td>
                <td className="px-4 py-3 text-muted-foreground">-</td>
                <td className="px-4 py-3">Yes</td>
              </tr>
              <tr className="border-b">
              ...
              <tr>
                <td className="px-4 py-3 font-mono text-xs">sparkline</td>
                <td className="px-4 py-3 text-muted-foreground">number[]</td>
                <td className="px-4 py-3 text-muted-foreground">[]</td>
                <td className="px-4 py-3">No</td>
              </tr>
