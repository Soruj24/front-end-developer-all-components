function DonutPieChart({ data, size = 160, innerRadius = 35 }: { data: { label: string; value: number; color: string }[]; size?: number; innerRadius?: number }) {
  const cx = size / 2;
  const cy = size / 2;
  const r = size / 2 - 10;
    ...
    <div className="rounded-xl border border-border p-6 dark:border-border">
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          <DonutPieChart data={donutData} size={140} innerRadius={35} />
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-lg font-bold text-zinc-800 dark:text-zinc-200">{donutData[0].value}%</span>
          </div>
        </div>
