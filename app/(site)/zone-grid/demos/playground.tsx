...
import { BasicGrid, GridWithLabels, GridColorZones, GridSpanVariants, GridResponsive } from "./grid-demos";

const VARIANTS = ["basic", "labels", "colors", "spans", "responsive"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicGrid />}
        {variant === "labels" && <GridWithLabels />}
        {variant === "colors" && <GridColorZones />}
        {variant === "spans" && <GridSpanVariants />}
        {variant === "responsive" && <GridResponsive />}
      </div>
    </div>
  );
}
