...
import { BasicSkew, SkewDirections, PerspectiveSkew, InteractiveSkew, SkewCards } from "./skew-demos";

const VARIANTS = ["basic", "directions", "perspective", "interactive", "cards"] as const;
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4 flex justify-center">
        {variant === "basic" && <BasicSkew />}
        {variant === "directions" && <SkewDirections />}
        {variant === "perspective" && <PerspectiveSkew />}
        {variant === "interactive" && <InteractiveSkew />}
        {variant === "cards" && <SkewCards />}
      </div>
    </div>
  );
}
