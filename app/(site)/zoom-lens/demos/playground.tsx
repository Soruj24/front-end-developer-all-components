...
import { BasicLens, LensSizes, LensShapes, LensWithGrid, LensWithZoom } from "./lens-demos";

const VARIANTS = ["basic", "sizes", "shapes", "grid", "zoom"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicLens />}
        {variant === "sizes" && <LensSizes />}
        {variant === "shapes" && <LensShapes />}
        {variant === "grid" && <LensWithGrid />}
        {variant === "zoom" && <LensWithZoom />}
      </div>
    </div>
  );
}
