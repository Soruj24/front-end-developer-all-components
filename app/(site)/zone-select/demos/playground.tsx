...
import { BasicSelect, SelectWithIcons, SelectCompact, SelectWithActions, SelectWithDescription } from "./select-demos";

const VARIANTS = ["basic", "icons", "compact", "actions", "description"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicSelect />}
        {variant === "icons" && <SelectWithIcons />}
        {variant === "compact" && <SelectCompact />}
        {variant === "actions" && <SelectWithActions />}
        {variant === "description" && <SelectWithDescription />}
      </div>
    </div>
  );
}
