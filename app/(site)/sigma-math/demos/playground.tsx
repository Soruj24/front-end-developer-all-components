...
import { FormulaVariant, EquationVariant, StatsVariant, CalculatorVariant, SumVariant, GridVariant } from "./playground-variants";

const VARIANTS = ["formula", "equation", "stats", "calculator", "sum", "grid"] as const;
type Variant = (typeof VARIANTS)[number];

export function PlaygroundDemo() {
  const [variant, setVariant] = useState<Variant>("formula");
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4 flex justify-center">
        {variant === "formula" && <FormulaVariant />}
        {variant === "equation" && <EquationVariant />}
        {variant === "stats" && <StatsVariant />}
        {variant === "calculator" && <CalculatorVariant />}
        {variant === "sum" && <SumVariant />}
