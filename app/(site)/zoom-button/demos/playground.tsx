...
import { BasicZoom, ZoomWithLabels, ZoomControls, ZoomSizes, ZoomStates } from "./zoom-demos";

const VARIANTS = ["basic", "labels", "controls", "sizes", "states"] as const;
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4 flex justify-center">
        {variant === "basic" && <BasicZoom />}
        {variant === "labels" && <ZoomWithLabels />}
        {variant === "controls" && <ZoomControls />}
        {variant === "sizes" && <ZoomSizes />}
        {variant === "states" && <ZoomStates />}
      </div>
    </div>
  );
}
