...
import { BasicHeader, WithActions, WithTabs, MinimalHeader, WithBadge } from "./header-demos";

const VARIANTS = ["basic", "actions", "tabs", "minimal", "badge"] as const;
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4 flex justify-center">
        {variant === "basic" && <BasicHeader />}
        {variant === "actions" && <WithActions />}
        {variant === "tabs" && <WithTabs />}
        {variant === "minimal" && <MinimalHeader />}
        {variant === "badge" && <WithBadge />}
      </div>
    </div>
  );
}
