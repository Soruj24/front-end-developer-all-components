...
import { BasicCard, CardVariants, CardWithProgress, CardCompact, CardWithPassword, CardList } from "./zip-demos";

const VARIANTS = ["basic", "variants", "progress", "compact", "password", "list"] as const;
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        ...
        {variant === "variants" && <CardVariants />}
        {variant === "progress" && <CardWithProgress />}
        {variant === "compact" && <CardCompact />}
        {variant === "password" && <CardWithPassword />}
        {variant === "list" && <CardList />}
      </div>
    </div>
  );
}
