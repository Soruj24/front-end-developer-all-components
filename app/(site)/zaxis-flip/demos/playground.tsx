...
import { BasicFlip, FlipDirections, FlipSpeeds, FlipCardSizes, FlipWithClick } from "./flip-demos";

const VARIANTS = ["basic", "directions", "speeds", "sizes", "click"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicFlip />}
        {variant === "directions" && <FlipDirections />}
        {variant === "speeds" && <FlipSpeeds />}
        {variant === "sizes" && <FlipCardSizes />}
        {variant === "click" && <FlipWithClick />}
      </div>
    </div>
  );
}
