...
import { BasicExtract, ExtractWithProgress, ExtractTree, ExtractWithSize, ExtractWithPreview } from "./extract-demos";

const VARIANTS = ["basic", "progress", "tree", "size", "preview"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicExtract />}
        {variant === "progress" && <ExtractWithProgress />}
        {variant === "tree" && <ExtractTree />}
        {variant === "size" && <ExtractWithSize />}
        {variant === "preview" && <ExtractWithPreview />}
      </div>
    </div>
  );
}
