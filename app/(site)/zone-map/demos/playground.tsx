...
import { BasicMap, MapWithColors, MapWithPins, MapCompact, MapWithLegend } from "./map-demos";

const VARIANTS = ["basic", "colors", "pins", "compact", "legend"] as const;
...
export function PlaygroundDemo() {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="mt-4">
        {variant === "basic" && <BasicMap />}
        {variant === "colors" && <MapWithColors />}
        {variant === "pins" && <MapWithPins />}
        {variant === "compact" && <MapCompact />}
        {variant === "legend" && <MapWithLegend />}
      </div>
    </div>
  );
}
