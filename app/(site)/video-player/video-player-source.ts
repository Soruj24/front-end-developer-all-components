export const VIDEO_PLAYER_SOURCE = `"use client";
...
export function VideoPlayerDemo() {
  ...
  return (
    <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 bg-black shadow-lg dark:border-zinc-700">
      <div className="relative group" onMouseEnter={() => setShowControls(true)} onMouseLeave={() => setShowControls(!playing)}>
        ...
        <div className={\`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-4 pb-3 pt-10 transition-all duration-300 \${showControls ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}\`}>
          <input type="range" min="0" max="100" value={progress} onChange={handleSeek} className="w-full h-1 appearance-none rounded-full bg-zinc-600 accent-white cursor-pointer" />
          <div className="mt-1.5 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <button className="rounded-lg p-1.5 text-white/80 transition-all hover:bg-white/10 hover:text-white active:scale-90" aria-label="Skip back"><SkipBack className="h-4 w-4" /></button>
              <button className="rounded-lg p-1.5 text-white/80 transition-all hover:bg-white/10 hover:text-white active:scale-90" aria-label="Skip forward"><SkipForward className="h-4 w-4" /></button>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button className="rounded-lg p-1.5 text-white/80 transition-all hover:bg-white/10 hover:text-white active:scale-90" aria-label="Settings"><Settings className="h-4 w-4" /></button>
