export const PHONE_INPUT_SOURCE = `"use client";
...
type PhoneInputValidation = "none" | "valid" | "invalid";
...
const SIZE_CLASSES: Record<PhoneInputSize, string> = {
  sm: "h-9 text-sm",
  md: "h-10 text-sm",
  lg: "h-12 text-base",
};
    ...
    return (
      <div className={cn("w-full max-w-sm space-y-1.5", className)}>
        ...
        >
          ...
          {endIcon && <span className="pr-3 text-muted-foreground">{endIcon}</span>}
          {!endIcon && validation === "valid" && <span className="pr-3"><Check className="h-4 w-4 text-emerald-600" /></span>}
          {!endIcon && validation === "invalid" && <span className="pr-3"><AlertCircle className="h-4 w-4 text-red-500" /></span>}
          {!endIcon && validation === "none" && <span className="pr-3"><Phone className="h-4 w-4 text-muted-foreground/60" /></span>}
        </div>
