export const TARGET_CARD_SOURCE = `"use client";
    ...
    <div className="w-full max-w-sm overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-950">
      ...
      <div className="space-y-4 px-5 py-4">
        ...
        <div className="h-2.5 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800">
          <div className={\`h-full rounded-full transition-all duration-500 \${ (current / target) >= 1 ? "bg-emerald-500" : (current / target) >= 0.75 ? "bg-blue-500" : "bg-orange-500" }\`} style={{ width: \`\${Math.min(100, (current / target) * 100)}%\` }} />
        </div>
