...
export function DescendingList() {
  ...
  const sorted = [...INITIAL_ITEMS].sort((a, b) => (a[sortKey] < b[sortKey] ? 1 : -1));
  ...
  return (
    <div className="w-full max-w-sm overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-950">
      <div className="flex items-center gap-2 border-b border-zinc-100 px-5 py-3 dark:border-zinc-800">
        <SortDesc className="h-4 w-4 text-zinc-400 dark:text-zinc-500" />
        <span className="text-xs font-semibold text-zinc-900 dark:text-zinc-100">Descending Sort</span>
      </div>
      <div className="flex gap-1.5 border-b border-zinc-100 px-5 py-3 dark:border-zinc-800">
        {(["name", "price"] as const).map((key) => (
          <button key={key} onClick={() => setSortKey(key)} className={`rounded-lg px-3.5 py-1.5 text-xs font-medium capitalize transition-all active:scale-95 ${sortKey === key ? "bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900" : "bg-zinc-100 text-zinc-500 hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:hover:bg-zinc-700"}`}>
            {key}
          </button>
        ))}
      </div>
