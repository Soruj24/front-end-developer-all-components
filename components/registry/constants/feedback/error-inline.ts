...
export const errorInline: RegistryEntry = entry({
    id: "error-inline",
    title: "Inline Alerts",
    description: "Dismissible error, warning, info, and success alert banners.",
    ...
    <div className="flex w-full flex-col gap-3">
      {visible.map((b) => (
        <div key={b.type} className={\`flex items-center justify-between rounded-lg border px-4 py-3 text-sm \${b.color}\`}>
          ...
          <button
            onClick={() => setDismissed((d) => [...d, b.type])}
            className="text-current opacity-60 hover:opacity-100"
            aria-label={\`Dismiss \${b.type}\`}
          >
            &times;
          </button>
        </div>
