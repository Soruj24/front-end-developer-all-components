...
export const popoverFlag: RegistryEntry = entry({
    id: "popover-flag",
    title: "Flag",
    description: "Quick flag actions for tracking and reporting.",
...
function FlagContent() {
  ...
  return (
    <div className="flex flex-col gap-1">
      <p className="text-xs font-medium text-zinc-500">Flag</p>
      {flags.map((f) => (
        <button key={f} onClick={() => setSel(sel.includes(f) ? sel.filter((x) => x !== f) : [...sel, f])} className={\`rounded px-2 py-1.5 text-left text-sm \${sel.includes(f) ? "bg-indigo-50 text-primary dark:bg-indigo-900/20" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}\`}>{f}</button>
      ))}
    </div>
  );
}

export default function PopoverFlag() {
  const [open, setOpen] = useState(null);

  return (
    <div className="flex w-full flex-wrap items-start justify-center gap-8">
      <div className="relative flex items-center justify-center">
        <button onClick={() => setOpen(0)} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs font-medium transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">Flag</button>
