...
export const popoverActionMenus: RegistryEntry = entry({
    id: "popover-action-menus",
    title: "Action Menus",
    description: "Action lists for menu, row, and more-action triggers.",
...
function MenuContent() {
  return (
    <div className="flex flex-col gap-1">
      <p className="mb-1 text-xs font-medium text-zinc-500">Actions</p>
      {["Edit", "Duplicate", "Archive", "Delete"].map((a) => (
        <button key={a} className="rounded px-2 py-1.5 text-left text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800">{a}</button>
      ))}
    </div>
  );
}
...
function TableActionsContent() {
  return (
    <div className="flex flex-col gap-1">
      <p className="text-xs font-medium text-zinc-500">Row Actions</p>
      {["View", "Edit", "Duplicate", "Delete"].map((a) => (
        <button key={a} className={\`rounded px-2 py-1.5 text-left text-sm \${a === "Delete" ? "text-danger hover:bg-danger-soft" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}\`}>{a}</button>
      ))}
    </div>
  );
}
...
export default function PopoverActionMenus() {
  ...
  return (
    <div className="flex w-full flex-wrap items-start justify-center gap-8">
      ...
      <div className="relative flex items-center justify-center">
        <button onClick={() => setOpen(1)} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs font-medium transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">Table Actions</button>
        <Popover open={open === 1} onClose={() => setOpen(null)} placement="left">
          <TableActionsContent />
        </Popover>
      </div>
      <div className="relative flex items-center justify-center">
        <button onClick={() => setOpen(2)} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs font-medium transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">More</button>
