...
function ReceiptRow({ label, note, value, tone = "default" }: { label: React.ReactNode; note?: string; value: string; tone?: "default" | "muted" | "discount" }) {
  return (
    <div className={cn("flex items-baseline gap-2 text-sm", tone === "muted" && "text-muted-foreground", tone === "discount" && "text-success")}>
      <span className="shrink-0">{label}{note && <span className="ml-1 text-xs text-muted-foreground">{note}</span>}</span>
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">{factor > 1 ? "billed annually" : "per month"}</span>
      </div>
      ...
      {showSavingsMeter && breakdown.discountsTotal > 0 && (
        <div className="mt-1 flex flex-col gap-1.5">
          <div className="flex items-center justify-between text-xs"><span className="text-muted-foreground">Savings</span>          <span className="font-medium text-success">{formatPrice(breakdown.discountsTotal, currency, locale)}<span className="ml-1 text-muted-foreground">({savingsPct}%)</span></span></div>
          ...
          {factor > 1 && <p className="text-[11px] text-muted-foreground">That&apos;s like getting {monthsFree} months of list price free.</p>}
        </div>
      )}
      <div className="mt-2 flex flex-col gap-3 border-t border-border pt-4">
        <div className="flex items-end justify-between gap-3">
          <div className="flex flex-col">
            ...
            {factor > 1 && <span className="text-[11px] text-muted-foreground">≈ {formatPrice(breakdown.monthlyTotal, currency, locale)}/mo</span>}
          </div>
