...
export function PaletteRow({ row, index, selected, query, favorites, pinned, onSelect, onToggleFavorite, onTogglePinned, onMouseEnter }: {
  ...
  return (
    <div role="option" aria-selected={selected} data-row-index={index} onMouseEnter={onMouseEnter} onClick={() => onSelect(row)}
      className={cn("group flex w-full cursor-pointer items-center gap-3 rounded-lg px-2.5 py-2 text-sm transition-colors", selected ? "bg-primary-soft text-foreground" : "text-foreground", item.disabled && "cursor-not-allowed opacity-50")}
      style={{ animation: "fade-slide 180ms cubic-bezier(0.16, 1, 0.3, 1) both", animationDelay: `${Math.min(index, 12) * 14}ms` }}>
      <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", item.icon ? "bg-muted text-foreground" : "bg-muted/60 text-subtle")}>
      ...
      {hasChildren && <ChevronRightIcon className={cn("h-3.5 w-3.5 shrink-0 text-subtle", selected && "text-primary")} />}
      <span className="flex shrink-0 items-center gap-0.5">
        <button type="button" aria-label={isFavorite ? `Remove ${item.label} from favorites` : `Add ${item.label} to favorites`}
          onClick={(e) => { e.stopPropagation(); onToggleFavorite(item.id); }}
          className={cn("rounded-md p-1 transition-colors", isFavorite ? "text-warning" : "text-subtle opacity-40 hover:text-warning hover:opacity-100")}>
        ...
        <button type="button" aria-label={isPinned ? `Unpin ${item.label}` : `Pin ${item.label}`}
          ...
          className={cn("rounded-md p-1 transition-colors", isPinned ? "text-primary" : "text-subtle opacity-40 hover:text-primary hover:opacity-100")}>
