...
import type { QuickNavProps, QuickNavItem } from "./QuickNav.types";

const QuickNav = forwardRef<HTMLDivElement, QuickNavProps>(
  (
    {
      open,
      onClose,
    ...
    useEffect(() => {
      if (selected > 0 && listRef.current) {
        const btn = listRef.current.querySelector(`[data-index="${selected}"]`);
    ...
    const content = (
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] sm:pt-[20vh]">
        ...
        <div
          ref={ref}
          className={cn(
            "relative z-50 flex w-full max-w-md flex-col overflow-hidden rounded-xl border border-border/60 bg-background shadow-2xl shadow-black/10 dark:shadow-black/30",
        ...
        >
          <div className="flex items-center gap-3 border-b border-border/60 px-4 py-3">
            ...
            />
            <kbd className="pointer-events-none hidden rounded-md border border-border/60 bg-muted/50 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground/60 sm:inline-block">
              ESC
            </kbd>
          </div>
          ...
          <div ref={listRef} className="max-h-72 overflow-y-auto p-2" role="listbox">
            ...
            {filtered.length === 0 && (
              <p className="px-3 py-8 text-center text-sm text-muted-foreground/60">No results found.</p>
            )}
          </div>
