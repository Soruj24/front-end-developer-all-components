  ...
  return (
    <>
      {searchable && (
        <div className="pointer-events-none absolute inset-x-0 top-0 flex justify-center p-3 sm:p-4">
          <div className="pointer-events-auto flex w-full max-w-xs items-center gap-2 rounded-xl border border-border bg-surface/95 px-3 py-2 shadow-card backdrop-blur">
            ...
            <input value={search} onChange={(e) => onSearchChange(e.target.value)} placeholder="Search nodes…" spellCheck={false} aria-label="Search graph nodes" className="w-full bg-transparent text-sm text-foreground outline-none placeholder:text-subtle" />
            {searchActive ? (
              <button type="button" onClick={onSearchClear} aria-label="Clear search" className="shrink-0 rounded-md p-0.5 text-muted-foreground transition-colors hover:text-foreground"><CloseIcon className="h-3.5 w-3.5" /></button>
            ) : (
              <span className="shrink-0 rounded-md border border-border bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">{nodesLength} nodes</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-3 left-3 flex items-center gap-1 rounded-xl border border-border bg-surface/95 p-1 shadow-card backdrop-blur">
        <button type="button" onClick={onZoomIn} aria-label="Zoom in" className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><PlusIcon className="h-4 w-4" /></button>
        <button type="button" onClick={onZoomOut} aria-label="Zoom out" className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><MinusIcon className="h-4 w-4" /></button>
        <button type="button" onClick={onFitAll} aria-label="Fit graph" className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><FitIcon className="h-4 w-4" /></button>
      ...
      {minimap && (
        <div className="absolute bottom-3 right-3 hidden h-24 w-32 overflow-hidden rounded-xl border border-border bg-surface/95 p-1 shadow-card backdrop-blur sm:block sm:h-28 sm:w-40">
          <svg ref={minimapRef} viewBox={`0 0 ${MINI_W} ${MINI_H}`} className="h-full w-full cursor-pointer touch-none select-none" onPointerDown={onMiniPointerDown} onPointerMove={onMiniPointerMove} onPointerUp={onMiniPointerUp} onPointerCancel={onMiniPointerUp} aria-hidden="true">
            <rect width={MINI_W} height={MINI_H} className="fill-transparent" />
            {minimapGeo.dots.map((d) => <rect key={d.id} x={d.x} y={d.y} width={Math.max(2, NODE_W * minimapGeo.s)} height={Math.max(2, NODE_H * minimapGeo.s)} rx={2} className="fill-foreground" opacity={0.55} />)}
            <rect ref={minimapViewportRef} x={0} y={0} width={40} height={40} rx={4} className="fill-primary/10 stroke-primary" strokeWidth={1.2} pointerEvents="none" />
          </svg>
        </div>
      )}
    </>
