    ...
    <div className="flex h-44 flex-col border-t border-border bg-background">
      <div className="flex items-center justify-between border-b border-border px-3 py-1.5">
        <span className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
          <TerminalIcon className="h-3.5 w-3.5" />Console<span className="font-mono text-[10px] text-subtle">{logs.length} entries</span>
        </span>
      ...
      <div className="scrollbar-thin flex-1 overflow-auto px-3 py-2 font-mono text-xs leading-relaxed">
        {logs.length === 0 ? <p className="text-subtle">Log output will appear here</p> : logs.map((log) => (
          <div key={log.id} className="flex items-start gap-2 border-b border-border/40 py-1">
            <span className="shrink-0 text-[10px] text-subtle">{new Date(log.ts).toLocaleTimeString([], { hour12: false })}</span>
            <span className={cn("shrink-0 text-[10px] font-semibold uppercase", log.type === "error" ? "text-danger" : log.type === "warn" ? "text-warning" : log.type === "info" ? "text-info" : log.type === "debug" ? "text-subtle" : "text-muted-foreground")}>{log.type}</span>
