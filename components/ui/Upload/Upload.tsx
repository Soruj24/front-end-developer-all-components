...
export function Upload({ accept, multiple = false, maxSize: _maxSize, onUpload, children, className }: UploadProps) {
  ...
  return (
    <div className={cn("inline-flex", className)}>
      ...
      <button type="button" onClick={() => inputRef.current?.click()} className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors duration-150">
...
export function UploadDropzone({ accept, multiple = false, maxSize: _maxSize, onDrop, className }: UploadDropzoneProps) {
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" accept={accept} multiple={multiple} onChange={handleChange} className="hidden" />
      <svg className="mb-2 h-8 w-8 text-muted-foreground/50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
