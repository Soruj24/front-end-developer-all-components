...
export function SpotlightSearch({ items, open: openProp, defaultOpen = false, onOpenChange, placeholder = "Search files, apps, actions...", emptyMessage = "No results found", storageKey = "spotlight-search", recentLimit = 5, width = 560, maxHeight = 420, bindShortcut = true, className }: SpotlightSearchProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { setRendered(false); setClosing(false); previous?.focus(); }, 160);
  ...
  if (prevOpen !== isOpen) { setPrevOpen(isOpen); if (isOpen) { setRendered(true); setClosing(false); setQuery(""); setSelected(0); } else setClosing(true); }
  ...
  const selectItem = useCallback((item: SpotlightItem) => { recordRecent(item.id); item.onSelect?.(); setOpenState(false); }, [recordRecent, setOpenState]);

  const sections = useMemo(() => buildSections(items, query, recents), [items, query, recents]);
  ...
  const handleKeyDown = useCallback((event: ReactKeyboardEvent<HTMLInputElement>) => {
    ...
    else if (event.key === "Enter") { event.preventDefault(); const row = rows[safeSelected]; if (row) selectItem(row); }
  ...
  const handlePanelKeyDown = useCallback((event: ReactKeyboardEvent<HTMLDivElement>) => {
    ...
    const focusables = Array.from(panelRef.current?.querySelectorAll<HTMLElement>("[data-focusable]") ?? []);
    if (focusables.length === 0) return;
    const current = document.activeElement as HTMLElement | null;
    const index = focusables.indexOf(current as HTMLElement);
    const next = event.shiftKey ? (index <= 0 ? focusables[focusables.length - 1] : focusables[index - 1]) : (index >= focusables.length - 1 ? focusables[0] : focusables[index + 1]);
    next?.focus();
  }, []);
  ...
  useEffect(() => { if (isOpen && rendered && !closing) inputRef.current?.focus(); }, [isOpen, rendered, closing]);
  useEffect(() => { if (!rendered || closing) return; const el = listRef.current?.querySelector(`[data-option-index="${safeSelected}"]`); el?.scrollIntoView({ block: "nearest" }); }, [safeSelected, rendered, closing, rows.length]);

  if (!rendered) return null;
  return (
    <div role="dialog" aria-modal="true" aria-label="Spotlight search" className={cn("fixed inset-0 z-50 flex items-end justify-center sm:items-start sm:px-4 sm:pb-8 sm:pt-[12vh]", className)}>
      <div className="fixed inset-0 bg-black/45 backdrop-blur-[2px]" style={{ animation: closing ? "fade-out 140ms ease both" : "fade-in 150ms ease both" }} onClick={() => setOpenState(false)} aria-hidden="true" />
      <div ref={panelRef} onKeyDown={handlePanelKeyDown} className="relative z-10 w-full overflow-hidden rounded-t-2xl border border-border bg-surface shadow-modal sm:rounded-2xl" style={{ width, maxWidth: "100%", animation: closing ? "zoom-out 150ms ease both" : "pop 220ms cubic-bezier(0.34, 1.56, 0.64, 1) both" }}>
        <SpotlightSearchBar query={query} setQuery={setQuery} setSelected={setSelected} inputRef={inputRef} placeholder={placeholder} handleKeyDown={handleKeyDown} />
        <SpotlightResults listRef={listRef} sections={sections} rows={rows} safeSelected={safeSelected} query={query} emptyMessage={emptyMessage} maxHeight={maxHeight} selectItem={selectItem} setSelected={setSelected} />
        <SpotlightFooter />
      </div>
    </div>
  );
