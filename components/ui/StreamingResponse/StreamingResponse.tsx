...
export function StreamingResponse({
  ...
  return (
    <div className={cn("overflow-hidden rounded-2xl border border-border bg-background shadow-lg shadow-black/5", variant === "plain" && "rounded-none border-0 bg-transparent shadow-none", className)}>
      {showHeader && (
        <header className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-border/60 px-4 py-3">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><SparkleIcon className="h-4 w-4" /></span>
          ...
          <div className="flex items-center gap-1.5">
            {busy ? (<ActionButton label="Stop generation" danger onClick={stop}><StopIcon className="h-3 w-3" />Stop</ActionButton>) : (
              <>
                <ActionButton label="Copy response" onClick={copy}>{copied ? <CheckIcon className="h-3.5 w-3.5 text-emerald-500" /> : <CopyIcon className="h-3.5 w-3.5" />}{copied ? "Copied" : "Copy"}</ActionButton>
      ...
      <div className="relative">
        ...
        {locked && (
          <button type="button" onClick={scrollToBottom} className="absolute bottom-3 right-3 z-10 inline-flex h-8 items-center gap-1.5 rounded-full border border-border bg-background/90 px-3 text-xs font-medium text-muted-foreground shadow-lg backdrop-blur-sm transition-colors hover:bg-muted hover:text-foreground" style={{ animation: "fade-slide 0.2s ease-out both" }}>
