...
export function TerminalEmulatorBody({
  lines,
  typingLine,
  promptSpans,
  buffer,
  ...
  return (
    <div
      ...
      className="scrollbar-thin flex-1 select-text overflow-y-auto px-4 py-3.5 text-[13px] leading-relaxed outline-none"
    ...
    >
      {lines.map((line) =>
        line.kind === "prompt" ? (
          <div
            key={line.id}
            className="flex flex-wrap items-baseline whitespace-pre-wrap break-words"
          >
        ...
        <div
          ...
          style={{
            color: typingLine.color
              ? resolveColor(typingLine.color, theme)
              : theme.fg,
          }}
        >
          {typingLine.text}
      ...
      <div className="flex items-center gap-0">
        ...
        <input
          ...
          className="min-w-0 border-0 bg-transparent p-0 font-mono text-[13px] leading-relaxed outline-none disabled:opacity-50"
          style={{ color: theme.fg, caretColor: theme.accent, maxWidth: "100%" }}
        />
        {!focused && (
          <span
            className="animate-pulse"
            style={{ color: theme.fg, fontWeight: 300 }}
            aria-hidden="true"
          >
            &#9613;
          </span>
        ...
        {busy && (
          <span
            className="ml-2 shrink-0 rounded bg-white/5 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider"
