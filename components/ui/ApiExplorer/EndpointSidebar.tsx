...
export function EndpointSidebar({
  ...
  return (
    <div className="hidden min-h-0 flex-col border-r border-border lg:flex">
      <div className="border-b border-border p-2">
        <div className="relative">
          ...
          <input
            ...
            className="h-8 w-full rounded-lg border border-border bg-background pl-8 pr-8 text-xs text-foreground outline-none transition-colors placeholder:text-subtle focus:border-primary"
          />
          <kbd className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 rounded border border-border bg-muted px-1.5 text-[10px] text-subtle">
            /
          </kbd>
        </div>
      </div>
      ...
      >
        {visible.length === 0 ? (
          <p className="px-2 py-4 text-center text-xs text-subtle">No endpoints match “{query}”.</p>
        ) : (
          <EndpointList
            endpoints={visible}
            cursor={cursor}
