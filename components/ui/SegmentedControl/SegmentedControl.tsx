...
import type { SegmentedControlProps, SegmentedControlSize } from "./SegmentedControl.types";
  ...
  ({ options, value, onChange, size = "md", className }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          "relative inline-flex items-center rounded-lg border border-border/60 bg-muted/50 p-1",
          "dark:bg-muted/30",
          className,
        )}
        role="radiogroup"
      ...
      >
        <span
          className="absolute top-1 bottom-1 z-0 rounded-md bg-background shadow-sm transition-all duration-200 ease-out"
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              key={opt.value}
              ref={(el) => { if (el) buttonsRef.current.set(opt.value, el); }}
              type="button"
              role="radio"
              ...
              className={cn(
                "relative z-10 flex items-center justify-center gap-1.5 rounded-md font-medium transition-colors duration-150 outline-none",
                s.container,
                s.button,
                isActive
                  ? "text-foreground"
